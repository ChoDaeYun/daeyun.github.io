...
구글검색에서 워낙 많이들 검색이 되고 있어 굳이 언급할 필요는 없을 듯 합니다. <br>

IOS 앱 내에서 로그인 아이디 비밀번호 확인이 끝난뒤에 진행되는 흐름은 다음 이미지와 같습니다. 
...
1. Authorization code 값 인증 하기  - 로그인 또는 회원가입시 인증에 사용되는 코드 ( 일회성으로 API호출시 새로운 Authorization code 가 발급됩니다. (api 호출하면 accesstoken 값이 authoriztion code 값이다 ))
2. refresh token 검증하기  - 서비스 중 애플에서 연동이 된 계정이 맞는지 확인할때 사용된다.  하루에 한번 정도만 호출하라고 가이드가 되어 있다. 


Generate and validate tokens URL : <a href="https://developer.apple.com/documentation/sign_in_with_apple/generate_and_validate_tokens">애플 가이드</a>
